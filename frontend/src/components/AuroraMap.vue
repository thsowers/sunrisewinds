...
function updateMap() {
  ...
  if (store.viewline.length > 0) {
    const latLngs: L.LatLngExpression[] = store.viewline.map((p) => [
      p.lat,
      p.lon,
    ]);
  ...
  if (store.tonightViewline && store.tonightViewline.viewline.length > 0) {
    const latLngs: L.LatLngExpression[] = store.tonightViewline.viewline.map(
      (p) => [p.lat, p.lon],
    );
    tonightViewlineLayer = L.polyline(latLngs, {
      ...
      .bindTooltip(`Tonight's viewline (Kp ${store.tonightViewline.max_kp})`, {
        sticky: true,
      })
      .addTo(map);
  }
  ...
  if (userLocation.value) {
    ...
    userMarker = L.marker([userLocation.value.lat, userLocation.value.lon], {
      icon,
    })
      .bindPopup(
        `<b>${userLocation.value.name}</b><br>${userLocation.value.lat}°N, ${userLocation.value.lon}°W`,
      )
      .addTo(map);
  }
}
